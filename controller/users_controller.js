...
async function signup(req, res) {
    try {
        ...
        db.query("UPDATE users SET ? WHERE id = ?", [{refreshToken: hashingToken}, insertId])
...
async function login(req,res) {
    try {
        ...
        await db.query("UPDATE users SET refreshToken = ? WHERE id = ?", [hashedRefresh, user.id])
...
async function refresh(req,res) {
    try {
        ...
        res.json({refreshToken, access_token})
    } catch (error) {
        res.status(error.status || 500).json({error: error.message})
    }
}
